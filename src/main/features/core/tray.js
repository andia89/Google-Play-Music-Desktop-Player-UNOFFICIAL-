...
const createTray = () => {
  ...
  Settings.onChange('appIconInvert', (newValue) => {
    ...
    if (appIcon && !appIcon.isDestroyed()) appIcon.setImage(getAppIconFileName());
  });
  ...
  const setContextMenu = (track) => {
    ...
    if (appIcon && !appIcon.isDestroyed()) appIcon.setContextMenu(contextMenu);
  };
  setContextMenu(null);
  ...
  if (appIcon && !appIcon.isDestroyed()) appIcon.setToolTip('Google Play Music Desktop Player');
